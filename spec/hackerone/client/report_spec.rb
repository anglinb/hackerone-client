...
RSpec.describe HackerOne::Client::Report do
  ...
  describe '#on_state_change_hooks' do
    ...
    it 'triggers hooks on state changes' do
      described_class.add_on_state_change_hook lambda { |_report, _old_state, _new_state|
        # NOOP, just leaving it hear to demonstrate the multiple hooks run
      }
      described_class.add_on_state_change_hook lambda { |report, _old_state, new_state|
        report.assign_to_user 'esjee' if new_state == 'triaged' && report.assignee.nil?
      }
